...
def run_convex():
        list_node.sort(key= lambda s:s.y,reverse=True)
        for item in list_node:
            print("x = "+str(item.x)+", y = "+str(item.y))
        x1, y1 = int(list_node[0].x)-2, int(list_node[0].y)-2
        x2, y2 = int(list_node[0].x)+2, int(list_node[0].y)+2
...
btn_run = Button(master, text="Run",command = run_convex)
btn_run.pack()
